...
SUBJECT_CONFIG = {
    "열역학": {
        ...
        # ★★ **2026-08-18 다시 켰다** — 사용자: [사용자 발화 인용 생략]. 그 화면에 표 읽는 법 튜토리얼을 두라는 요청이라,
        #   꺼 두면 **아무도 못 가는 자리에 설명을 두는 꼴**이 된다.
        #   · 옛 판정(2026-08-06 [사용자 발화 인용 생략] — 문풀·연습문제와 함께 숨김)의 근거는
        #     «검수 안 한 것을 내보내지 않는다» 였다. 그 근거는 2026-08-14 에 다른 과목들에서
        #     이미 뒤집혔다([사용자 발화 인용 생략]).
    ...
    # ★ 2026-08-14 사용자 판정으로 **다시 연다** — [사용자 발화 인용 생략].
    #   숨기던 근거(2026-08-06, [사용자 발화 인용 생략])가
...
#   강의계획서는 **교재 폴더**에 과목마다 있다(`google drive_excess/2. 전공과목/<학기>/<과목>/`).
...
#   「없다」가 아니라 「거기까지는 없다」»* 를 그대로 저질렀다. 사용자가 [사용자 발화 인용 생략] 로 짚어 줬다. **0건을 낼 때는 어디까지 봤는지 먼저 적는다.**
#   현재 교양은 **하나뿐이다**(사용자 확인: [사용자 발화 인용 생략]).
_TAXONOMY = {
...
#   그런데 같은 날 사용자가 [사용자 발화 인용 생략] 라고 **홈**을 짚었다 —
...
def local_ports(vbs_text, subject_by_branch, subject_names):
    """serve_site.vbs 의 브랜치→포트를 **과목 폴더 이름→포트**로 옮긴다. 순수 함수 — 테스트 대상.

    `SUBJECT_BY_BRANCH` 의 값은 접두어다(`math` → `공학수학`, 실제 폴더는 `공학수학 1`).
    그래서 실재하는 이름과 접두 매칭해서 옮긴다.
    ★ 브랜치 이름에 **숫자**가 들어간다(`math2`) — `[a-z]+` 이면 그 과목만 조용히 빠진다.
    """
    out = {}
    for branch, port in ((m[0], int(m[1])) for m in VBS_PORT_RE.findall(vbs_text or "")):
        prefix = subject_by_branch.get(branch)
        if not prefix:
            continue
...
def lint_baseline(baseline, ch_path, rev):
    ...
    **그 과거 데이터가 오늘 규칙에 걸려** 빌드가 죽었다(유니코드 첨자 24건 — 방금 우리가 고친 바로 그것들).
    ...
    그런데 이 플래그의 존재 이유가 정확히 [사용자 발화 인용 생략] 다 —
    기준선이 낡을수록 필요하고, 낡을수록 못 잡히는 셈이었다. 사용자 지적:
    [사용자 발화 인용 생략] 가 이 구조 때문이다.
...
def build_and_record(template, subject, cfg, ch_path, chapter_nav, review_enabled,
    ...
    빠지고, 빠진 챕터는 [사용자 발화 인용 생략] 가 되어 그 다음 수락이
...
def main():
    ...
    #   (2026-08-13 신설). [사용자 발화 인용 생략] 의 기준은 **시각이 아니라 커밋**이다 — 검수 중에 들어온
    ...
    #   사용자가 **신뢰의 문제**로 말했다([사용자 발화 인용 생략]).
    ...
    #   AGENTS 「알려진 함정」의 [사용자 발화 인용 생략] 와 같은 자리이고,
    ...
    if merged_tree and os.path.isfile(merged_tree):
        ...
        try:
            ...
            local_tree = deploy_all.local_course_tree(
                deploy_all.deployable_worktrees(deploy_all.worktrees()))
            if local_tree:
    ...
    #   [사용자 발화 인용 생략]). 사이드바는 «이 과목만» 으로 좁혔지만
    ...
    #     같은 산출물이 배포에서 못 쓰인다. 사용자 확인: [사용자 발화 인용 생략].
    ...
    # ★★ 검수 보류 — 사용자가 [사용자 발화 인용 생략] 라고 한 동안은 기준선을 못 민다 (신설 2026-08-13).
    #
    # 실사고: 2026-08-12 검수 배치에서 사용자가 [사용자 발화 인용 생략] 라고 못 박았는데, 인박스의 「남은 순서」 7번에
    # [사용자 발화 인용 생략] 이 **대기**로 적혀 있었고 세션을 닫으며 그것을 실행했다.
    # 그 결과 회차 1~12 의 수정 **84항목**이 화면에서 통째로 사라졌다(ch01 24·ch02 33·ch03 3·
    # ch04 5·ch05 17·ch06 2). 사용자가 다음 세션에 [사용자 발화 인용 생략] 로 발견했다.
    #
    # 구조가 원인이다 — **보류는 채팅에만 있고 계획은 파일에 있었다.** 파일이 이긴다.
    # 그래서 보류를 **파일에 적고 기계가 읽게** 한다. 푸는 방법은 우회 플래그가 아니라
    # `index.json` 에서 `reviewHold` 를 **지우는 것**이다 — 그게 곧 사용자의 [사용자 발화 인용 생략] 이고,
    ...
    #
    #   실사고: 사용자가 [사용자 발화 인용 생략] 라고 한 **시각**의 HEAD 를 잡았는데, 그 10분 전에
        ...
        for subject, cfg, idx, chapter_nav in subjects_meta:
            for entry in idx.get("chapters", []):
                ...
                if not os.path.isfile(ch_path):
                    continue
                if (accept_review_chapter
                        and os.path.splitext(entry["file"])[0] != accept_review_chapter):
                    continue
    ...
    for subject, cfg, idx, chapter_nav in subjects_meta:
        for entry in idx.get("chapters", []):
            ...
            if not os.path.isfile(ch_path):
                continue
            rel = os.path.relpath(ch_path, ROOT).replace(os.sep, "/")
            if accept_review_chapter and os.path.splitext(entry["file"])[0] != accept_review_chapter:
                ...
                built.append(out)
                print("[ok]", os.path.relpath(out, ROOT), ("(+mirror)" if mirror else ""))
                continue
    ...
    #   ★ 트리거를 **반드시 하는 일**(빌드)에 건다 — [사용자 발화 인용 생략]
    ...
    #   빌드도 회귀도 통과한다. [사용자 발화 인용 생략] 를 규칙으로 적어 두는
