...
# 0개가 나왔는데 사용자는 계속 *[발화 생략]* 고 했다. 규격의 바닥이 낮았던 것이다.
...
def label_sizes(svg_line):
    ...
    열린 날 2026-07-30 (ch02~05 이관 중). 이 함수는 원래 *[발화 생략]* 이라는 주석을
...
def _my_subject():
    """이 실행이 어느 과목을 보는가. **환경변수 → 갈래** 순으로 본다.

    ★★ **갈래로 묻는 것은 죽은 전제다** (고친 날 2026-09-08). 이 함수는 갈래 이름에서
      과목을 찾았는데, 2026-09-07 평탄화로 갈래가 `main` 하나가 되면서 **어느 과목도
      못 찾는 자**가 됐다 — 그래서 이 도구를 순회기에 물리면 21과목 전부 `exit 2` 였고,
      「삽화 여백 253건」이 **도구가 있는데도 손으로 고칠 일**로 남아 있었다.
      평탄화 뒤의 규약은 `SUBJECT` 환경변수이고(`for_each_subject.py` 가 그것을 준다),
      `inspect_data`·`audit_content` 가 이미 그 규약으로 돈다. 여기만 옛 전제에 남아 있었다.
    ☐ 갈래 조회는 **지우지 않고 뒤에 둔다** — 과목 갈래를 다시 쓰는 곳이 있으면 그대로 돈다.
    """
    env = (os.environ.get("SUBJECT") or "").strip()
    if env:
        return env
