...
왜 있나 (열린 날 2026-07-31, 사용자 지적):
    *"치수보조선의 경우 치수선. 화살표를 만난 후 거기서 조금 더 지나가야해"*
    *"치수 보조선도 물체 표면에서 얼마나 떨어져야 하는지에 대한 규칙이 아직 없는 것 같은데.
      스프링이랑 볼트랑 치수보조선 떨어진게 값이 달라"*

    AGENTS 는 *"약간 띄운다"* 라고만 적고 **수치가 없었다.** 그래서 삽화마다 사람이 눈으로
...
def _replace_line(svg, old, new):
    ...
    ★ `<line>` 과 `<path>` 를 **둘 다** 본다. 처음엔 `<line>` 만 봤는데 실측에서 11건이
      *"`<line>` 을 못 찾았다"* 로 빠졌다 — 같은 보조선을 `<path d='M264 224V270'/>` 로
