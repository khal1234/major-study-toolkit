...
★ 왜 열렸나 (2026-08-13). 사용자가 `ℓ` 하나를 두고 *"좀 더 내려"* 를 세 번 말했고, 그 과정에서
...
from buildlib.checks_svg import (                                          # noqa: E402
    DIM_LABEL_GAP_MIN_EM, _attr, _svg_texts, _tagged_group_spans, _text_bbox,
)
# ★ 챕터 파일은 **표기 보존 기록기**로만 쓴다 — 통째로 다시 쓰면 손으로 압축해 둔 SVG 표기가
#   전부 재포맷된다. 이 부류는 `fix_dim_label` 로 한 번 재발했고, 그래서 `test_checks` 가
#   *"새 도구는 처음부터 write_chapter 를 쓸 것"* 을 계약으로 잠갔다.
