...
*"`du_θ/dt = -θ ur – θ 방향이 돌아가면 중심 쪽으로 기웁니다` 여기서 - 이건 왜 불편하게
느껴지지 그 위에 dur얘네도 그렇고"*. 같은 사용자가 **바로 앞 문장에서** 절 제목의 줄표
(*"중심에서 잴 때 – 극·원통 성분"*)는 *"이건 괜찮은데"* 라고 했다 — 즉 줄표 자체가 아니라
**수식 뒤에 붙은 줄표**가 문제다.

★ 왜 수식 뒤에서만 불편한가: 바로 앞에 `= -\dot\theta\,\mathbf{u}_r` 처럼 **진짜 마이너스**가
  있으면 눈은 같은 높이의 짧은 가로줄을 하나 더 보고 **연산자로 읽는다.** 산문 뒤라면
  그렇게 읽힐 여지가 없다. 그래서 판정선은 *"앞이 `\)` 인가"* 하나다.
★ 처방이 콜론인 이유: 이 자리의 문형은 **`수식(레이블) → 설명(값)`** 이고, AGENTS 표기 세부가
  *"레이블→값은 콜론, 대등 나열만 가운데점"* 이라고 이미 정해 두었다. 새 규격이 아니라
  **적용을 빠뜨린 자리**다.
★ 손으로 고치지 말 것: 실측 동역학 3챕터에 11곳이었고, 고치면서 **콜론 뒤 공백을 빠뜨리기 쉽다**
  (실제로 그렇게 한 번 났다 — AGENTS *"문장부호 뒤에는 공백 한 칸"*). 그래서 이 도구는
