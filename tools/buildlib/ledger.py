...
★ 열린 날 2026-08-06 — 사용자 지적:
  *"합의는 있었는데 ⑴ AGENTS에 적히지 않았고 ⑵ 검사가 없습니다 — 이거 앞으로 얘기할 거에
  대해 합의는 되는데 또 1, 2 저 과정을 안 해서 일이 생길 수도 있는 거 아닌가?"*

**맞다. 그리고 그날 실제로 터졌다.** C31(이해도 체크의 답에 뒤 장을 넣지 않는다)은 인박스에
*"기계 방지 후보: …"* 로만 적혀 있었고, 그래서 인스턴스는 사라졌지만 **다시 써도 막는 것이
없었다.** 같은 상태의 항목이 그날 실측으로 **26곳**이었다(원장 6 · 인박스 18 · 워크오더 2).

AGENTS 는 이미 *"「기계」 칸이 빈 채로 항목을 닫지 않는다"* 고 못 박아 두었다.
...
def _shared_root():
    """나루(공용 시스템) 폴더 — 있으면 Path, 없거나 못 읽으면 `None`. 순수 함수가 아니라 IO다.

    ★ 열린 날 2026-09-01 — 나루가 **자기 자신을 대상으로 닫은** 첫 원장 행에서 드러났다
    (`도구/check_relay.py`, 기계 칸이 나루 자신을 가리킴). `source_blob` 이 이 리포의
    `tools/`·`.claude/hooks/` 만 훑어서, **실재하는 장치인데 유령으로 신고됐다.**
    나루 자신의 `도구/`·`훅/` 도 「실행 증거」 후보에 넣는다 — `.claude/hooks/shared_sync_check`
...
# 나루 자신을 대상으로 닫은 원장 행의 「기계」가 살 수 있는 곳 — `SOURCE_DIRS` 와 같은 형태이되
...
#   *"여기는 더 이상 정본이 아니다 (2026-08-16, 사용자 판정)"* 가 적혀 있다.
#   같은 함수를 두 파일에 걸어 잰 실측: **사본(395줄) 0건 · 정본(863줄) 3건.**
#   그래서 `test_ledger_closed_rows_name_a_real_machine` 과 `close_report` 의 「승격 잔량」이
#   **둘 다 정본을 안 봐서 영원히 초록**이었다 — 폴백을 든 자가 *"한 건도 안 봤다"* 를
#   *"0건"* 으로 찍는, 이 리포가 `audit_problem_originality.PDF_FOR` 에서 이미 한 번 닫은 부류다.
...
def resolve_ledger():
    ...
    나루가 없는 배치에서는 사본으로 떨어져야 한다(그 기계에서 자가 죽으면 안 된다).
...
def ledger_source_note(text=None):
    ...
    ★★ **`text` 를 주면 「분모」를 같이 낸다** (열린 날 2026-08-26). 부류 이름은 나루가
    ...
    if LEDGER_IS_CANONICAL:
        ...
        note = ("★ **정본을 못 봤다** — 나루 「기록/feedback-ledger.md」 를 못 찾아 사본(%s)으로 "
...
#
# ★ `후보` 는 뺐다 (2026-08-06, 첫 실행의 실측으로 정정). 이 리포는 *"L2 후보 경고"* 처럼
#   **검사 이름 안에서도** 그 낱말을 쓰기 때문에, 후보를 실패 신호로 삼으면 이미 구현된 행이
#   무더기로 걸린다. 실제로 잔량이 부풀었다. 미완을 뜻하는 말은 `미구현`·`미정` 이다.
UNCLOSED_WORDS = ("미구현", "미정")
# 검사로 만들 수 없다고 **명시**한 행. AGENTS 규칙 11 은 *"검사할 수 없는 것은 미검증이라고
# 밝힌다"* 를 허용하므로, 이렇게 적고 대체 조치를 남긴 행은 닫힌 것으로 본다.
# 조용히 비워 두는 것과 못 한다고 적는 것은 다르다 — 이 자가 가르려는 것이 정확히 그 차이다.
NOT_MECHANIZABLE = ("기계 판정 불가", "만들지 않았다", "만들지 않는다")
# ★ **부분 닫힘**은 닫힘이 아니다 (2026-08-06 실측으로 정정). 이 원장은 *"인스턴스는 닫혔고
#   검사는 열림"* 을 `부분 닫힘` 으로 정직하게 적어 왔는데, 상태 칸에 '닫힘' 글자가 들어 있어
...
#   *"결함이 아니었다"* 로 닫은 행이 섞여 있다(실측 3건). 그런 행에 검사 이름을 요구하면
...
def declared_unmechanizable(cell):
    ...
    근거를 달아 *"만들지 않았다"* 고 판정한 것도 여기 든다(실측 1건뿐이라 레지스트리를 세우지
...
#     실물: 2026-08-26 행이 지목한 훅 함수 하나(나루 `훅/gate_rerun_guard.py`).
...
def source_paths(root=None):
    ...
    ★ 이 리포(`SOURCE_DIRS`)에 더해 나루 자신(`SHARED_SOURCE_DIRS`)도 **선택적으로** 훑는다 —
    나루를 대상으로 닫은 원장 행이 나루 자신의 도구를 지목할 수 있어서다. `root` 를 명시로 준
    호출(테스트가 임시 폴더로 대체하는 자리)은 나루를 안 본다 — 그 자리는 이 리포의 스캔 범위
    ...
    if root is None:
        ...
        if shared is not None:
            for rel, exts in SHARED_SOURCE_DIRS:
                ...
                files, size = files + n, size + b
                mark = " **0개 — 이 폴더를 못 봤다**" if not n else ""
                parts.append("나루/%s %d개%s" % (rel, n, mark))
