...
REVIEW_COLLECTIONS = ("theory", "derivation", "practice", "problems")
...
#   ⑵ 그리고 그 상태를 `.claude/SUBJECT.md` 에 [사용자 발화 인용 생략] 이라고 적어,
#      다음 세션이 **문서를 근거로** 그대로 두게 만들었다.
#   ⑶ `build_review` 의 출력이 **합계뿐**이라(`본문변경 22`) 그 22 안에 이론 10절이 들어 있다는
#      것이 화면에 안 보였다. 합계는 컬렉션을 감춘다.
#   → 같은 부류가 AGENTS 에 이미 [사용자 발화 인용 생략] 로 적혀 있다. 네 번째다.
#
# 그래서 **수락 범위를 스냅샷 옆에 기록**한다. 이게 있어야 close 때 [사용자 발화 인용 생략] 을 기계가 지목할 수 있다(`stale_acceptances`).
...
def viewer_changes_for(ch_path, current_rev, log_path=VIEWER_CHANGE_LOG):
    ...
    ★★ **열린 날 2026-08-23, 사용자 지적:** [사용자 발화 인용 생략] — 맞다. 변경점 판정의 입력이 **`chNN.json` 스냅샷 하나**라
...
def _record_acceptance(ch_path, only, sections, formulas, baseline_rev=None, viewer_rev=None):
    ...
    #   두면 [사용자 발화 인용 생략] 를 나중에 물을 수 없다. 오늘 사고의 진단이
...
#
# 실사고: 사용자가 [사용자 발화 인용 생략] 라고 한 **시각**의 HEAD(`36fc657`)를 ch05 기준선으로
...
#   관측할 수 없다. 그래서 `history` 에 최근 빌드 sha 를 남겨, 거부 출력이 [사용자 발화 인용 생략] 를
...
def record_build(ch_path, rev=None, dirty=None):
    ...
    그 사실을 함께 적어야 나중에 [사용자 발화 인용 생략] 는 거짓 결론을 막는다.
    """
...
def commits_touching(ch_path, old_rev, new_rev, log=None):
    ...
    [사용자 발화 인용 생략] 을 [사용자 발화 인용 생략] 로 읽는 것이 반복된 실패다(AGENTS 규칙 11).
    """
    if log is not None:
...
def baseline_move_findings(ch_path, target_rev, working_tree=False, record=None,
    ...
    #   표시 자체가 없고(=사용자가 잃을 마크가 없고), AGENTS 도 [사용자 발화 인용 생략] 며 미검수 챕터를 HEAD 로 밀라고 한다. 여기서 막으면 그 정상
    #   동작이 통째로 잠긴다.
...
def unacknowledged_swallows(findings, acknowledged=None):
    ...
    ★ 커밋만 sha 로 승인된다. `unbuilt`·`dirty`·`uncommitted` 는 **플래그로 못 푼다** —
      처방이 [사용자 발화 인용 생략] 라서, 그 자리에 우회로를 내면 규칙이 곧 꺼진다
...
def marks_by_collection(marked_chapter):
    ...
    합계만 보면 [사용자 발화 인용 생략] 안에 이론 10절이 들어 있는지 알 수 없다 — 그 눈멂이 이 부류의
    재발 경로였다(위 주석 ⑶).
...
def visible_collections(chapter):
    ...
    판정 기준은 뷰어와 같다 — 화면에서 안 보이는 것을 [사용자 발화 인용 생략] 으로 올리면
...
def stale_acceptances(ch_path, chapter, marks, touched=None):
    ...
    ★★ **수락 뒤에 새로 고친 것과 구별해야 한다** (2026-08-07, 첫 구현의 결함).
      처음엔 [사용자 발화 인용 생략] 로 짰는데, 그러면 **지적을 받아 고친 배치마다
      울린다.** 그 표시는 잘못이 아니라 [사용자 발화 인용 생략] 라는 보고이고,
...
#
#   [사용자 발화 인용 생략] — 실측하니 그 절의 독자 화면은 한 글자도
...
#       `sourceRef`  — 뷰어가 [사용자 발화 인용 생략] 고 명시하고 데이터에만 남긴다.
...
def _svg_texts_only(value):
    ...
    [사용자 발화 인용 생략] 인데, SVG 원문끼리 비교하면 font-size 하나만 바뀌어도
...
#   [사용자 발화 인용 생략] 로 신고했다. 같은 배치의 **이론 절 둘은
#   정상으로 잡혔다** — 컬렉션마다 명단이 갈려서 생긴 구멍이다.
#   2026-07-21 에 같은 모양을 이미 한 번 닫았다: [사용자 발화 인용 생략]. 그때는 **검사**였고 이번은 **변경점 판정**이라
...
def author_asked_to_show(old, new):
    ...
    ★ 선언은 `changeNote` 하나로 족하다. 이미 [사용자 발화 인용 생략] 를 적는
...
def _mark_changed_figure(item, old_item, changes, levels):
    ...
    ★★ **열린 날 2026-08-19.** 사용자: [사용자 발화 인용 생략] — 맞는 말이고, 이 자료에는 **그 장치가 이미 있다**(변경점 표시).
    ...
    # ★★ **어느 조각이 바뀌었는지까지 준다** (2026-08-19, 사용자 3회차: [사용자 발화 인용 생략]).
...
def _compare_review_items(current, previous, fields, summary):
    ...
    if missing:
        # ★ `--quiet` 에서는 **개수만** 낸다 (2026-08-12, 사용자: [사용자 발화 인용 생략]). 이 경고를 넣은 그 회차에 출력이 15줄 늘어
...
def add_review_changes(ch, ch_path, enabled):
    ...
    **한 번도 검수하지 않은 컬렉션에 변경점을 쌓는 것은 그 자체로 잘못이다.** AGENTS:
    [사용자 발화 인용 생략] 나중에 그 탭을 열면 어차피 전부를 읽는다.
