...
#   그건 **공용 폴더의 폴더 이름**이라 이 리포(`docs/`)로 가져오자마자 «선언이 없다» 로 죽었다.
...
def selftest():
    ...
    # ★ 폴더 이름을 박으면 가져간 쪽에서 죽는다 (2026-08-26 실사고 — 공용 폴더 `기록/` 를 박아 뒀다).
    chk("★ 선언 자리는 **훑는다** — 공용 폴더 `기록/` 와 이 리포 `docs/` 둘 다",
    ...
    h = parse_history("# 머리\n측정 2026-08-18 공용 폴더=5/12 편입=4/4\n측정 2026-08-25 공용 폴더=9/12 편입=4/4\n")
    chk("대장의 **마지막** 줄만 읽는다", h == {"공용 폴더": 9, "편입": 4}, str(h))
