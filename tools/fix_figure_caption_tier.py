...
사용자 지적: *"'서로 독립인 상태량 2개 → 점 하나로 상태가 고정' 얘네나 '1→2 (경로 A)',
'다른 경로로 처음 상태로 되돌아오는 닫힌 루프 = 사이클' 이런것들 글자 크기가 좀 크다."*
