...
증상이 조용하다:
  ...
  - `.gitignore` 누락 → 빌드 산출물이 커밋에 섞인다. **실제로 난 적 있다** —
    `.claude/SUBJECT.md` 에 *"아직 안 바꾼 과목은 이 조치를 먼저 하라(공학수학은 완료)"* 로
    사후 조치가 남아 있다.

사용자 요구(2026-07-28): *"내가 뭘 입력해야 하는지만 물어봐주고 만드는 건 너가 알아서 하는 구조."*
...
#
# 이 자리에는 한때 *"등록 지점으로 두지 않는다"* 고 적혀 있었다. 근거는 원장 14-b 가
# `additionalDirectories` 에 상위 `2. 전공과목` 을 통째로 올려 *"등록 절차 자체를 없앴다"* 는
...
def add_pitfall_book(src, subject, book):
    ...
    ★ 왜 늘었나 (열린 날 2026-08-15). 2-2 과목 일곱을 `register` 로 세웠더니 회귀가
      **7건 한꺼번에** 떴다 — *"등록 과목 '유체역학' 의 교재 접두어가 있다"*. 이 도구는
...
# 새 브랜치 세션은 **그중 하나도 못 받았다.** 사용자 지적: *"같이 질문과 답변으로
# 인지시켜줬는데 못 받은 거면 시스템적으로 수정 필요하겠는데."*
#
# **원인은 빠뜨림이 아니라 구조다.** 이 도구는 빈 템플릿을 복사하고
# *"내용은 그 브랜치 세션에서 채운다"* 고 안내했는데, 그 세션은 **아직 존재하지 않고
...
def render_subject_md(subject, branch, facts):
    ...
    out = ["# SUBJECT.md — " + subject + " (" + branch + " 전용)", "",
           "> 과목 무관 규칙은 `AGENTS.md`(main 정본)에 있다. 여기에는 **이 과목의 사실**만 둔다.",
...
def cmd_drop_inherited(args):
    ...
    그 규칙의 의도는 *"남의 과목 콘텐츠를 이 세션이 고치지 못하게"* 인데, 여기서 하는 일은
