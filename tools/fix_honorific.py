...
#   ★ 2026-09-08 보강 — **모음으로 끝나는 어간의 형용사** 셋을 더한다.
#     표 셀에 말투를 묻기 시작하자 `비싸다`·`느리다`·`모자라다` 가 처음 걸렸고,
#     기본값(명사 추측)이 `비싸입니다`·`느리입니다`·`모자라입니다` 를 만들었다.
#     겉모양으로는 `길이다`(명사)와 못 가른다 — 가르는 것은 낱말이라 목록이 정본이다.
#     ☐ 이 목록은 늘어난다. `--table` 의 「명사로 추측」 줄이 그 입구다 — 거기 뜬 것이
#       서술어면 여기 적고, 명사면 그대로 둔다.
_MU_PREDICATE = {"아니": "아닙니다", "크": "큽니다", "바쁘": "바쁩니다", "나쁘": "나쁩니다",
                 "비싸": "비쌉니다", "느리": "느립니다", "모자라": "모자랍니다"}
...
def convert_svg(svg):
    ...
    ★ **자와 같은 범위만 본다.** `_svg_texts` 는 검사(`figure_tone_issues`)가 쓰는 바로 그
      함수라, **굵은 제목**(표제와 같은 부류라 말투를 묻지 않는다)이 자동으로 같이 빠진다.
      처방이 자보다 넓으면 아무도 신고하지 않는 곳을 조용히 고치게 된다 — 위 `SKIP` 주석의
      그 사고다.
      ★ 2026-09-08 에 **회전 글자가 이 범위 안으로 들어왔다.** 그전에는 자가 `transform`
        붙은 글자를 통째로 건너뛰어서 처방도 함께 빠져 있었는데, 그 제외가 없어지면서
        둘이 같이 넓어졌다 — **이 함수는 한 줄도 안 고쳤다.** 한 자를 공유한다는 것이
        그런 뜻이고, 자를 고치면 처방이 따라오는 것이 정상이다.
