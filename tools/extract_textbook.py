...
def stage_images(paths):
    ...
    막히는 일이 있다. 그때 *"게이트가 막아서 못 봤다"* 로 끝내면 **자료를 안 읽은 것**이고,
    사용자에게 *"열어서 알려달라"* 고 떠넘기게 된다 — `render_pages` 를 만든 이유와 **같은 부류**다.
