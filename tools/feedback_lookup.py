...
def shared_root():
    """공용 시스템(공용 폴더)의 자리. 옛 이름 `Claude` 도 본다."""
...
def find_ledger():
    ...
    ★★ **이식 첫날 이 자가 남의 리포에서 죽었다 (2026-08-16).** `기록/feedback-ledger.md`
      를 글자로 박아 뒀는데 전공정리는 그 원장이 `docs/` 에 있어 *[발화 생략]* 로 끝났다.
      아침에 `check_narration` 이 `ROOT.parent` 로 겪은 것과 **같은 부류**다 —
      **이식할 때 경로 전제를 다시 안 잰 것.** 이 저장소는 처방을 이미 갖고 있었다
      (`scan_private.LEDGER_NAMES` — *[발화 생략]*).

    ★ 찾는 순서가 곧 판정이다 — **정본이 먼저다.** 원장의 정본은 2026-08-16 부터 공용 폴더이고,
      다른 프로젝트가 이 자를 돌리는 목적은 *«프로젝트 사이 재발»* 을 보는 것이라
      자기 사본을 보면 뜻이 없다. 공용 폴더가 없는 기계에서만 자기 리포에서 찾는다.
    """
...
#   *[발화 생략]* 라고 한 지적을 조회하니 **최고 0.35(불일치)** 가 나왔다 —
...
def main():
    ...
    ledger = find_ledger()
    if ledger is None:
        sys.exit("원장을 못 찾았다 — 정본은 공용 폴더의 `기록/%s` 다"
