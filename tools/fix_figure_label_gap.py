...
왜 만들었나 (열린 날 2026-07-30):
    ...
    *"화살표에서 얼마나 띄울지 그 위치는 어디에 둘지 같은거 일관된 규칙이 없는거야?"* 라는
    지적이 열린 원인이다(캡션 위계에서 이미 같은 이유로 도구를 만들었다 —
    `fix_figure_caption_tier.py`).

    사용자 결정(2026-07-30): *"지금 잠깐 불편하다고 제대로 삽화 만드는 과정을 안거치면
    결국 나중가서 처음부터 다시 뒤집어야해."* → 도구로 맞추고, 새로 생긴 겹침은 검수로 잡는다.
...
def _alignment_cluster(texts, boxes, index, axis):
    ...
    ★ 왜 필요한가 (2026-07-30 첫 시안이 실제로 이걸 깼다):
      ...
      AGENTS 「여백·라벨 규격」이 *"같은 역할의 라벨은 같은 규칙으로 배치한다"* 로 금지한 것이고,
      애초에 이 규격이 생긴 이유(*"경로 A는 선과 떨어져 있는데 경로 B는 왜 선에 겹쳐 있어"*)와
    ...
    ★ 처음엔 '중심 거리 4em 안'이라는 반경을 걸었다가 **부류를 절반 놓쳤다**(2026-07-30 실측):
      ...
      고치려던 그 결함(*"경로 A는 선과 떨어져 있는데 경로 B는 왜 선에 겹쳐 있어"*)을 새로 만든 것이다.
...
def _straddles(box, shapes):
    ...
    ★ 첫 시안이 이걸 빠뜨려 빌드가 2건을 잡았다(`fig-continuum-vs-rarefied` 'λ',
      ...
      AGENTS 「여백·라벨 규격」의 *"라벨을 경계선·테두리에 걸치게 두지 않는다"* 위반이고
...
def plan_figure(svg, fig_id="?"):
    ...
    #   *"감사와 같은 함수로 잰다. 따로 재면 갈라진다"* 라고 적어 둔 바로 그 갈라짐이다.
