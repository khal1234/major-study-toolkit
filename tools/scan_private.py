...
왜 (2026-08-12, 사용자):
*"공통 시스템 Claude 는 어찌 보면 **다른 사람들에게 그대로 전달해도 무방하게** 하고 싶은 거야.
그러니 내 이름이나 주소 뭐 이런 개인정보는 들어가면 안 돼. 근데 내가 뭐 공부했고 거기서 뭐가
문제가 되었고 이런 거는 크게 문제는 안 될 듯싶어. … 결국에 **시행착오를 통해 만들어낸
결과치**잖아. 이걸 공유하면 다른 사람들이 고생을 덜할 수 있지 않을까 싶어서."*
...
## 판정선 — 지울 것과 남길 것
  ...
  **그게 이 폴더의 값어치 전부다** — 규칙만 있고 왜 열렸는지가 없으면
  ...
  ★★ **사용자 원문은 두 부류다 — 한 덩어리로 다루지 않는다** (2026-08-12 개정).
    처음엔 *"사용자 지적 원문은 남긴다. 개인 정보가 아니다"* 라고만 적었는데, 그 한 줄이
    **기술적 지적과 개인 상태를 같이 통과시켰다.** 실측: 원장에 **피로·집중 저하·기기 중단을 말한 1인칭 문장**이
    ...
    ★ **이 검사는 둘째 부류를 못 잡는다.** 정규식으로 가릴 수 있는 것이 아니다 —
      ...
      *"사생활도 깨끗하다"* 로 읽지 말 것(범위를 안 밝힌 0건은 「없다」가 아니다).
"""
import re
import subprocess
import sys
...
#   2026-08-25 (나루) — `토큰대` 하나. 「2만 토큰대」의 **수량 범위**이고 위 넷과 같은
...
#   2026-08-25 (나루) 열림: `지웠대` 를 시드로 처리하려다 열었다. 그 1건을 인박스에
...
#   *"실제로 읽은 파일 0개 … 그대로 넘겨도 되는 상태다"* 를 찍었다. 그 폴더는 **배포하는 자리**다.
...
SKIP_DIRS = {"__pycache__", ".git", "Songs", "Cache", "Themes", "NoteSkins",
             "_retired", "Save", "node_modules", ".venv"}
...
RULES = [
    ...
    ("학교·회사", re.compile(r"[가-힣]{2,4}(?:대학교|대학(?![가-힣])|고등학교)|(?<![가-힣])[가-힣]{2}대(?![가-힣])"),
     "지우거나 일반화"),
...
def partition(root):
    ...
    한 개 차이가 사소해 보이지만 이 도구의 출력 전체가 *"0건이 어디까지의 0건인가"* 를
...
def manifest_sources():
    ...
    ★ 기본 범위를 이걸로 잡는 이유: 이 검사의 목적은 *"남에게 넘겨도 되는가"* 이고,
...
def main():
    ...
    #   *"그대로 넘겨도 되는 상태다"* 였다. 범위는 위에 정직하게 찍혀 있었지만
