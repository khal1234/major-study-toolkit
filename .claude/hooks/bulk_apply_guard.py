#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""PreToolUse(Bash) — **되돌릴 수 없는 상태에서 대량 적용을 돌리지 않는다** (공용 폴더에서 받음 2026-08-16).

    settings.json 의 PreToolUse(Bash) 훅으로 건다.
    자기 점검:  python .claude/hooks/bulk_apply_guard.py --selftest

## 왜 열렸나 (`/insights` 154세션, 공용 폴더 원장 2026-08-16)
...
바깥 제안은 *[발화 생략]* 였는데 **그 5는 근거 없는 수**다(실행 규율 16).
...
이름 어림(`fix_*`)은 **안 쓴다** — 공용 폴더가 같은 날 두 번 오막음한 그 방식이다.
...
def segments(command):
    ...
    ★★ **이 자가 공용 폴더에서 첫 커밋에 자기를 막았다** (2026-08-16). 변경일지에 적을 글을
...
def measurable_root(root):
    ...
    ★★ **컨테이너 루트는 워크트리가 아니다** (전공정리 2026-08-16 — 이식하며 넓혔다).
      ...
      *"fatal: this operation must be run in a work tree"* 로 죽는다. 공용 폴더 원본을 그대로
...
REASON = (
    ...
    "  되돌리기가 한 명령이다.\n"
    "  ★ 공용 폴더 원장 2026-08-16: 기호 치환기가 부피 `V`·체적유량 `V̇`·SVG `path` 의 `V`\n"
...
def selftest():
    ...
    cases = [
        ...
        # ★ 이 자가 공용 폴더에서 첫 커밋에 자기를 막은 실제 명령 꼴이다 — 잠근다
        ("cat >> 변경일지.md <<'EOF'\n| 표 | `sed -i`·`--apply` 를 본다 |\nEOF",
    ...
    #   공용 폴더 원본은 여기서 `None` 을 받아 «안 막음» 이 됐다. `main/` 으로 물러나 실제로 잰다.
