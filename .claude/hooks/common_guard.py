...
# 사용자 지적: *[발화 생략]*
#
# **원인은 게이트가 아니라 에이전트의 위반이다.** AGENTS 실행 규율 4가 *[발화 생략]* 고 이미
...
#   · **쓰기는 안 넓어진다** — settings 의 `deny` 에 `Edit/Write(<교재 폴더>/**)` 가 그대로다.
...
#
# 사용자: *[발화 생략]* — `wscript tools/serve_site.vbs` 가 **두 번째로** 떴다
# (원장 18 이 같은 명령, dynamics 세션).
#
# **부류를 절반만 옮긴 것이 원인이다.** 위 원장 17 의 실측은 *[발화 생략]* 인데, 그건 디렉터리만의 성질이 아니다 — `allow` 규칙도 똑같이
...
#
# 18 번의 조치는 *[발화 생략]* 였는데 이 파일은 `.gitignore` 라
...
def main():
    ...
    if event in ("SessionStart", "UserPromptSubmit"):
        ...
        if added:
            notes.append(
                ...
                "아직 한 번 뜰 수 있다.")
        # ★ 2026-09-06 평탄화 뒤로 HEAD 가 곧 main 이라 이 수는 **항상 0** 이다 — 남겨 둔 것은
        #   과목별 갈래가 다시 생기면 그때 다시 울리게 하기 위해서다(지우면 그 자리가 비어 버린다).
        #   문구는 「받아라」가 아니라 **관측 사실**로 적는다 — 평탄화 뒤에 옛 처방을 그대로 두면
        #   다른 SUBJECT.md 들이 그랬듯 폐기된 절차가 현재 지시로 읽힌다(Codex 검토 F06).
        n = commits_behind_main(_git(["rev-list", "--count", "HEAD..main", "--"] + COMMON_PATHS))
        if n > 0:
            notes.append(
                "⚠️ 공통 코어(CLAUDE.md·.claude·tools·site/template·docs)가 main 에서 "
                + str(n) + "커밋 앞서 있다. **평탄화 뒤로는 0이어야 하는 수다** — 0이 아니면 "
                "갈래가 다시 생겼다는 뜻이니 무엇이 갈렸는지부터 확인할 것.")
        if notes:
            _emit(event, "\n".join(notes))
        return
    ...
    if event == "PreToolUse":
        ...
        if is_common_path(path):
            _emit("PreToolUse",
                  "공통 파일을 수정 중이다 — **저장하는 순간 21과목이 전부 이 규칙 아래 놓인다**"
                  "(2026-09-06 평탄화 이후 전파할 것이 없다). 새 규칙이면 그 자리에서 소급도 본다: "
                  "`python tools/audit_convention_drift.py` 로 이미 쓴 콘텐츠에 후보가 남았는지 훑는다"
                  "(CLAUDE.md 「공통 vs 과목별」)." + note)
        elif note:
