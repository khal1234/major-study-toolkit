#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""달마다 한 번 도는 것이 밀렸을 때만 한 줄 알린다 (공용 폴더에서 받음 2026-08-16).
...
# 고른 값 — 사용자 요청이 *[발화 생략]* 이라 **달의 길이**를 그대로 쓴다(28·30·31 중
...
def find_records():
    ...
    ★★ **공용 대장을 함께 읽는다** (사용자 판정 2026-08-17: *[발화 생략]*). `/insights` 는 **기계 전역**이라
      한 번 돌리면 어디서 돌렸든 끝난 것인데, 대장이 프로젝트마다 따로면 **나머지가 계속
      조른다.** 실측: 공용 폴더가 2026-08-16 에 154세션을 돌려 적었는데 이 리포는 「기록 없음」으로
      떠 있었다 — 알림이 사실과 어긋난 것이다.
    ★ 처방은 이 계통에 이미 있다 — `feedback_lookup` 이 *[발화 생략]* 로 같은 판정을 했다.
