...
[사용자 발화 인용 생략] 인데 실측은 **편집마다 한 번**이다. 그 규칙은 2026-08-12 에
있었고 **지켜지지 않았다.** 원인은 규칙이 없어서가 아니라 **재는 자만 있고 막는 자가
없어서**다 — 같은 날 실측: 재는 자 8개 중 `exit != 0` 을 내는 것은 **하나**뿐이었다.
AGENTS 17항이 스스로 적어 둔 함정이 이것이다: [사용자 발화 인용 생략]
...
2. **반대편 실패가 더 나쁘다.** 규율 13: [사용자 발화 인용 생략]
...
`commit.py` 의 [사용자 발화 인용 생략] 와 같다. 정말 다시 돌려야 하면 **트리를 건드린 뒤에 돌린다** — 그러면 조건이
...
- **잠금장치가 없다.** 나루엔 아직 회귀 하네스가 없다 — `--selftest` 가 그 자리를 임시로
...
def find_tools(root=None, deep=False):
    ...
    ★★ **자리를 짐작하지 않고 찾는다 — 같은 부류 9회차** (전공정리 이식 2026-08-16).
      나루 배치(`훅/`·`도구/`)가 글자로 박혀 있어 이 리포(`main/.claude/hooks/`)에서는
...
def tree_fingerprint(root):
    ...
    **안 바뀐 것으로 오판**한다 — 그러면 이 자가 정당한 재실행을 막는다.
    ...
    ★ 나루 원본이 [사용자 발화 인용 생략] 를 열어 뒀는데, 그 구멍의 가장 나쁜
...
def running_command_lines(timeout=4):
    ...
    ★★ **어림을 안 쓴다 — 실제로 도는지 본다** (나루 2026-08-16, 규칙 B).
    ...
    """
    if os.name == "nt":
        # ★ `wmic` 는 최신 윈도우에서 **빠졌다** — 나루 실측에서 0줄이 나왔다(2026-08-16).
...
def actually_runs(command, gate):
    ...
    ★★ **막는 자는 세는 자보다 엄해야 한다** (2026-08-16, 이 자가 첫날 나를 잘못 막았다).
      `gate_signature` 는 자기 독스트링에 [사용자 발화 인용 생략] 라고 적어 뒀는데
...
def verdict(command, root, session, now_fp=None, state=None, procs=None):
    ...
    #   ★ 이 파일이 이미 적어 둔 규율 그대로다: [사용자 발화 인용 생략]
    ...
    #   하필 [사용자 발화 인용 생략] 를 강제하는 자가 자기 대조군을
    ...
    # ☐ **못 보는 것:** 지문은 이 세션이 선 트리(나루)만 잰다. 남의 리포를 대상으로 도는
...
def selftest():
    ...
    #   파이썬**과 맞기 때문이다. 사용자가 그날 밤 [사용자 발화 인용 생략]
...
def check_wiring(target):
    ...
    #   나루의 `도구/` 가 이미 `sys.path` 에 있어서, 사본이 **없는** knu-bot 을 재는데
    #   나루 것을 읽고 «✔» 를 냈다 — 오늘 네 번 나온 그 부류의 **다섯 번째**이고,
    ...
    # ★★ **규칙 B 가 이 기계에서 실제로 읽나** (전공정리 2026-08-16). 나루가 `wmic` 이
